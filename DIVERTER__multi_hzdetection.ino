...
int ios = 2;          /// Number of SSR to control
...
{
 InitTimersSafe(); 
  boolean sta=false;                                  //an indicator to exit the while loop

  unsigned long start1 = millis(); 
  
  while(sta==false) {
  
 while((millis()-start1)<200 && analogRead(aIn)>512  );//wait for positive half period to expire
  while((millis()-start1)<200 && analogRead(aIn)<512 );//wait for negative half period to expire
...
if ( stat == 0){
  ...
  Serial.print("TaskValueSet,2,1,"); Serial.println(percent);  // Diverter Percentage 
     Ppulse = DIVERT;
     Ppulse=map(Ppulse,0,Pstep,0,65535);    
